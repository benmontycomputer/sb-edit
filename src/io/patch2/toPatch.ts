...
import * as patch from "./interfaces";
import { OpCode } from "../../OpCode";

const BIS = patch.BlockInputStatus;
...
export default function toSb3(project: Project, options: Partial<ToPatchOptions> = {}): ToPatchOutput {
  ...
  function serializeInputsToFields(
    ...
    fieldEntries: (typeof patch.fieldTypeMap)[OpCode]
  ): patch.Block["fields"] {
    ...
    const fields: patch.Block["fields"] = {};
  ...
  function serializeInputShadow(
    ...
    if (primitiveOrOpCode === BIS.BROADCAST_PRIMITIVE) {
      ...
      const fieldEntries = patch.fieldTypeMap[shadowOpCode];
      if (fieldEntries) {
  ...
  function serializeInputsToInputs<PassedInputs extends { [key: string]: BlockInput.Any }>(
    ...
    const resultInputs: patch.Block["inputs"] = {};

    for (const [key, entry] of Object.entries(inputEntries)) {
      const input = inputs[key];
      if (entry === patch.BooleanOrSubstackInputStatus) {
  ...
  function serializeInputs(
    ...
    inputs: patch.Block["inputs"];
    fields: patch.Block["fields"];
    mutation?: patch.Block["mutation"];
  } | null {
    ...
    const fields = serializeInputsToFields(block.inputs, patch.fieldTypeMap[block.opcode]);

    let inputs: patch.Block["inputs"] = {};
    let mutation: patch.Block["mutation"];
    ...
    if (block.isKnownBlock()) {
      switch (block.opcode) {
        case OpCode.procedures_definition: {
          ...
          const prototypeInputs: patch.Block["inputs"] = {};
        ...
        case OpCode.procedures_call: {
          ...
          const inputEntries: Record<string, patch.BlockInputStatus> = {};
          ...
          for (let i = 0; i < args.length; i++) {
            ...
            switch (type) {
              case "boolean":
                inputEntries[id] = patch.BooleanOrSubstackInputStatus;
        ...
        default: {
          const inputEntries = patch.inputPrimitiveOrShadowMap[block.opcode];
  ...
  interface SerializeTargetOptions {
    ...
    broadcasts: patch.Sprite["broadcasts"];
  }

  function serializeTarget(target: Target, options: SerializeTargetOptions): patch.Target {
    ...
    const blockData: patch.Target["blocks"] = {};
  ...
  function serializeSprite(sprite: Sprite, options: SerializeSpriteOptions): patch.Sprite {
  ...
  interface SerializeStageOptions {
    ...
    broadcasts: patch.Target["broadcasts"];
    tempo: number;
  ...
  function serializeStage(stage: Stage, options: SerializeStageOptions): patch.Stage {
  ...
  function serializeProject(project: Project): patch.ProjectJSON {
